<div align="center">

# Bingo 

Bingo, a new Bing that makes you breathe easier。

It highly restores the main operations of the New Bing web version, is available in China, is compatible with most Microsoft Bing AI functions, and can be deployed and used by yourself.

![Github stars](https://badgen.net/github/stars/Niansuh/bingo?icon=github&label=stars)

</div>

## Demo site

Site 1 (V2): https://bing.github1s.tk

Site 2(V1): https://copilot.github1s.tk

Site 3 (Agent): https://bingo.weaigc.repl.co (This mode is no longer recommended)

[![img](./docs/images/demo.png)](https://bing.github1s.tk)

## Functions and Features

- Supports unlimited conversations
-Support access from anywhere
-Support Docker build for easy and fast deployment and access
- Cookies can be configured globally and shared globally
-Support continuous voice conversation
-Supports use without account
- completely free
- Support OpenAI method calling [Usage Document](./OPENAI.md)
-Support independent deployment

## RoadMap
  - [x] Support wss forwarding
  - [x] Support one-click deployment
  - [x] Optimize mobile display
  - [x] Support drawing
  - [x] Support voice input (supports voice commands, currently only supports PC version of Edge and Chrome browsers)
  - [x] Support voice output (needs to be turned on manually)
  - [x] Support image recognition (gpt4 mode and default personality mode are not supported)
  - [x] Support custom domain names
  - [x] Support offline access
  - [x] Adapt to dark mode
  - [x] Support history
  - [x] Support built-in prompt words
  - [x] Support Workers deployment
  - [x] Support OpenAI API
  - [x] Support GPT4 mode
  - [ ] International translation

## Online deployment
#### Deploy to CodeSandbox (recommended)
Click [![One-click deployment to CodeSandbox](https://img.shields.io/badge/%E4%B8%80%E9%94%AE%E9%83%A8%E7%BD%B2-CodeSandBox- blue)](https://codesandbox.io/p/devbox/github/weaigc/bingo/tree/main?import=true), click “Fork” in the upper right corner, and then click “Next” all the way (no registered account Just try again after registering your account).

#### Deploy to Render
Thanks to [@SokWith](https://github.com/SokWith) for testing, Render is now resurrected.
To prevent account suspension, one-click deployment is no longer available. Please go to https://render.com to deploy manually.

### Proxy mode (not recommended)
When the previous methods cannot be used, you can use the proxy mode, then resolve the domain name to your own server, and continue to use it as a compromise.
#### 1. Deploy to Replit (recommended)

[Click to deploy Replit](https://replit.com/@weaigc/bingo?v=1)

#### 2. Deploy to CloudFlare (you need your own domain name)

- [Sign up for a Cloudflare account](https://dash.cloudflare.com/sign-up)

- To add a new website, you need to have your own domain name and host the domain name `Name Server` to Cloudflare (more information can be found on Google)

- Enter "Workers" through the left menu and click "Create a Worker".

- Create a Worker service, copy [worker.js](./cloudflare/worker.js) and paste all the code into the created service, save and deploy.

- Customize the access domain name in the trigger.

### Deploy other platforms
<details>
<summary>
<del>The following are the banned deployment methods</del>

The V2 version has solved this problem. For details, please see: https://github.com/weaigc/bingo/tree/v2
</summary>

#### Deploy to HuggingFace
1. Click this icon
[![Deploy to HuggingFace](https://img.shields.io/badge/%E7%82%B9%E5%87%BB%E9%83%A8%E7%BD%B2-%F0%9F% A4%97-fff)](https://huggingface.co/login?next=%2Fspaces%2FNiansuh%2FBing%3Fduplicate%3Dtrue%26visibility%3Dpublic), the configuration does not need to be changed.

2. After the deployment is completed, click "Settings" > "Site Domain Name", click once, copy the HF domain name information, and then share it with others.


#### Deploy to Netlify
[![Deploy to Netlify Button](https://www.netlify.com/img/deploy/button.svg)](https://app.netlify.com/start/deploy?repository=https://github .com/weaigc/bingo)

#### Deploy to Vercel
If you are a paid Vercel user, you can click the link below to deploy to Vercel with one click. The free version has [interface timeout limit](https://vercel.com/docs/concepts/limits/overview), which is not recommended.

[![Deploy with Vercel](https://vercel.com/button)](https://vercel.com/new/clone?demo-title=bingo&demo-description=bingo&demo-url=https%3A%2F% 2Fbing.github1s.tk%2F&project-name=bingo&repository-name=bingo&repository-url=https%3A%2F%2Fgithub.com%2Fweaigc%2Fbingo&from=templates&skippable-integrations=1&env=BING_HEADER&envDescription=%E5%A6%82%E6%9E %9C%E4%B8%8D%E7%9F%A5%E9%81%93%E6%80%8E%E4%B9%88%E9%85%8D%E7%BD%AE%E8%AF%B7 %E7%82%B9%E5%8F%B3%E4%BE%A7Learn+More&envLink=https%3A%2F%2Fgithub.com%2Fweaigc%2Fbingo%2Fblob%2Fmain%2F.env.example)

</details>

## Environment and dependencies

- Node.js >= 18
- New Bing's [User Information](#How to get-BING_HEADER))
- A vps that has not been banned by Microsoft (needs to be tested by yourself)

## Installation and use

> Local deployment requires that your local machine or server IP is a foreign IP and has not been banned by Microsoft, otherwise New Bing cannot be used normally.

* Start using Node

```bash
git clone https://github.com/weaigc/bingo.git
npm i
cd bingo
npm run build
npm run start
```

* Start using Docker
```bash
docker pull weaigc/bingo
docker run --rm -it -p 7860:7860 weaigc/bingo
# If it does not work properly, you can try setting the environment variable BING_HEADER
docker run --rm -it -e BING_HEADER=xxxx -p 7860:7860 weaigc/bingo
# If it still doesn't work, you can try proxy mode
```

## How to get BING_HEADER
> Configuring BING_HEADER means that you share your account with everyone who uses this service. If you do not need the login-free drawing function, it is not recommended to set this variable.

Open https://www.bing.com and log in, then visit https://www.bing.com/turing/captcha/challenge and pass the human-computer verification (if **Invalid Domain** is displayed, skip the verification test regardless), then

![BING HEADER](./docs/images/curl.png)

> The copied content should look like this. After confirming that the format is correct, open https://bing.github1s.tk/#dialog=%22settings%22, paste it in, click "Convert to BING_HEADER and copy", and then paste it from the clipboard to get it. (You can also verify on the web page first)

The following is a format reference. It should be noted that the format saved on the web page starts with `curl`, while the `BING_HEADER` configured on the server is in `base64` format, and the two cannot be interoperable.
<details>
<summary>Normal format/Format saved on the web page (format is for reference only)</summary>

```
curl 'https://www.bing.com/turing/captcha/challenge' \
  -H 'authority: www.bing.com' \
  -H 'accept: text/html,application/xhtml+xml,application/xml;q=0.9,image/webp,image/apng,*/*;q=0.8,application/signed-exchange;v=b3;q=0.7' \
  -H 'accept-language: zh-CN,zh;q=0.9,en;q=0.8,en-GB;q=0.7,en-US;q=0.6' \
  -H 'cache-control: max-age=0' \
  -H 'cookie: MicrosoftApplicationsTelemetryDeviceId=3399c004-fd0e-48ec-bb92-d82a27b2bbd4; _EDGE_V=1; SRCHD=AF=NOFORM; SRCHUID=V=2&GUID=29EBDDA4E6674329ACCF1A0A423C3E98&dmnchg=1; _UR=QS=0&TQS=0; _HPVN=CS=; _RwBf=ilt=1&ihpd=1&ispd=0&rc=0&rb=0&gb=0&rg=200&pc=0&mtu=0&rbb=0&g=0&cid=&clo=0&v=1&l=2023-07-25T07:00:00.0000000Z&lft=0001-01-01T00:00:00.0000000&aof=0&o=2&p=&c=&t=0&s=0001-01-01T00:00:00.0000000+00:00&ts=2023-07-25T11:00:31.7111548+00:00&rwred=0&wls=&lka=0&lkt=0&TH=&dci=0; ANON=A=0043C6590EA808ED6E395059FFFFFFFF&E=1c8b&W=1; NAP=V=1.9&E=1c31&C=DnaMSbDN_4efZ_xXqBF3Daorjr53kYqYoaP8YHsupjmiXnysX7a37A&W=1; PPLState=1; KievRPSSecAuth=; WLS=C=9df3f9d8518fae19&N=wen; WLID=pGY8HgWCu4p5XYCOk2oa0+DBdftkMUfmNIn8XtSjSTKsgv/Il7GUlYs0Jpjf/E12jZMgV7x44Dy3fXOgjjUoJx7Y/ClLrLhsk20THksJJoI=; _EDGE_S=F=1&SID=17CF6EE006426448213C7DB907436588&mkt=zh-CN; MUID=225621093D8A6C27301632413C0E6D08; MUIDB=225621093D8A6C27301632413C0E6D08; SUID=A; SNRHOP=I=&TS=; _U=nGyzKQruEsDwLiu65fZFIG6e12hf2lwTJmroW__k8joUJIKmG3OIjayXKGW9dCVR3sNhF76mEVxyW6yjUGPodOfjtSa3s3J_DxMOrEK1BqXCOBI9bC66spAIASV7prsYFlVAJz73jVNENp_tBubLHJy6EbT0BKRe4AjrYkH-9uMnmCKB8Zmyg; _SS=SID=17CF6EE006426448213C7DB907436588&R=0&RB=0&GB=0&RG=200&RP=0&PC=U531; SRCHS=PC=U531; USRLOC=HS=1&ELOC=LAT=22.501529693603516|LON=113.9263687133789|N=%E5%8D%97%E5%B1%B1%E5%8C%BA%EF%BC%8C%E5%B9%BF%E4%B8%9C%E7%9C%81|ELT=2|&CLOC=LAT=22.50153029046461|LON=113.92637070632928|A=733.4464586120832|TS=230726151034|SRC=W; SRCHUSR=DOB=20230725&T=1690384908000&POEX=W; ipv6=hit=1690388509974&t=6; SRCHHPGUSR=HV=1690384945&SRCHLANG=zh-Hans&PV=15.0.0&BRW=MW&BRH=MT&CW=410&CH=794&SCW=410&SCH=794&DPR=1.5&UTC=480&DM=0&WTS=63825879627&PRVCW=410&PRVCH=794&PR=1.5; cct=AjWIBYOoVP-Afq6gWwtx80If6yHn6iBuEVHA1XHdAKpny6Y_CVyi_MSyM94VyMWnjdYkkccVtm3czoIAtXUGQA; GC=AjWIBYOoVP-Afq6gWwtx80If6yHn6iBuEVHA1XHdAKpR3Y_D9Ytcks4Ht6XhadXk75dvhzP4YOUS0UmoEyqyxw' \
  -H 'dnt: 1' \
  -H 'sec-ch-ua: "Chromium";v="116", "Not)A;Brand";v="24", "Microsoft Edge";v="116"' \
  -H 'sec-ch-ua-arch: "x86"' \
  -H 'sec-ch-ua-bitness: "64"' \
  -H 'sec-ch-ua-full-version: "116.0.1938.29"' \
  -H 'sec-ch-ua-full-version-list: "Chromium";v="116.0.5845.42", "Not)A;Brand";v="24.0.0.0", "Microsoft Edge";v="116.0.1938.29"' \
  -H 'sec-ch-ua-mobile: ?0' \
  -H 'sec-ch-ua-model: ""' \
  -H 'sec-ch-ua-platform: "Windows"' \
  -H 'sec-ch-ua-platform-version: "15.0.0"' \
  -H 'sec-fetch-dest: document' \
  -H 'sec-fetch-mode: navigate' \
  -H 'sec-fetch-site: none' \
  -H 'sec-fetch-user: ?1' \
  -H 'sec-ms-gec: B3F47AD4A283CAB374C0451C46AAFD147C6A4DACAFF6A1C13F34B2C72B024494' \
  -H 'sec-ms-gec-version: 1-116.0.1938.29' \
  -H 'upgrade-insecure-requests: 1' \
  -H 'user-agent: Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/116.0.0.0 Safari/537.36 Edg/116.0.0.0' \
  -H 'x-client-data: ' \
  -H 'x-edge-shopping-flag: 1' \
  --compressed
```
</details>

<details>
<summary>Convert to the format after base64 (BING_HEADER can only use the format after base64)</summary>

```

```
</details>

### How to get BING_COOKIE

> Configuring BING_COOKIE means that you share your account with everyone who uses this service. If you do not need the login-free drawing function, it is not recommended to set this variable.

Open https://www.bing.com and log in, then visit https://www.bing.com/turing/captcha/challenge and pass the human-computer verification (if **Invalid Domain** is displayed, it means you need to prepare a ladder), then

![BING_COOKIE](./docs/images/curl.png)

> The copied content should look like this. After confirming that the format is correct, open https://bing.github1s.tk/#dialog=%22settings%22, paste it in, click "Convert to BING_COOKIE and copy", and then paste it from the clipboard to get it. (You can also verify on the web page first)


## Acknowledgments
  - Thanks to [EdgeGPT](https://github.com/acheong08/EdgeGPT) for providing the proxy API method.
  - Thanks to [Vercel AI](https://github.com/vercel-labs/ai-chatbot) for providing the basic scaffolding and [ChatHub](https://github.com/chathub-dev/chathub) [go-proxy -bingai](https://github.com/adams549659584/go-proxy-bingai) Part of the code provided.
  - Thanks to Happy-clo, SokWith, player868 and others in the group for their great assistance (the list is not complete, please point out if there are any omissions)


## Q&A and communication
[Telegram here](https://t.me/+NS5dCA4P-s9hYjNl)

<div align="left">
   <image src="./docs/images/wechat.jpg" width=200 />
   <image src="./docs/images/wechat2.jpg" width=200 />
   <image src="./docs/images/wechat3.jpg" width=200 />
</div>

## Star History

<a href="https://star-history.com/#Niansuh/bingo&Date">
   <picture>
     <source media="(prefers-color-scheme: dark)" srcset="https://api.star-history.com/svg?repos=Niansuh/bingo&type=Date&theme=dark" />
     <source media="(prefers-color-scheme: light)" srcset="https://api.star-history.com/svg?repos=Niansuh/bingo&type=Date" />
     <img alt="Star History Chart" src="https://api.star-history.com/svg?repos=Niansuh/bingo&type=Date" />
   </picture>
</a>

## License

MIT © [LICENSE](https://github.com/Niansuh/bingo/blob/main/LICENSE).
